...
This project uses **expo-dev-client**, not Expo Go. After `run:ios`, open the **Beacon** app (your icon), not the separate **Expo Go** app.
...
### App opens to Expo launcher instead of Beacon

You are in the **Beacon dev client** (correct app), but it is not connected to Metro yet.
...
3. Do **not** use Expo Go — use the **Beacon** app icon installed by `run:ios`.
...
1. Confirm Metro in a terminal: `npm start` — you should see `Metro waiting on exp+beacon://...` and port **8081**.
...
3. On the phone, in the **Beacon** dev launcher → expand **Enter URL manually** → connect to:
   ...
   ```text
   exp+beacon://expo-development-client/?url=http%3A%2F%2F192.168.1.94%3A8081
   ```

4. iOS **Settings → Beacon → Local Network** must be **On** (otherwise the phone cannot reach your Mac).
...
Build succeeded but install failed — iOS lockdown rejected the Mac’s pairing record (`InvalidHostID`). Not an app bug.
...
If it still fails, install from Xcode: open `ios/Beacon.xcworkspace`, select your device, press Run. Or use the simulator: `npx expo run:ios`.
